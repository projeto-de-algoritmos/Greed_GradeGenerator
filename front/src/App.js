...
function App() {
  ...
  return (
    <div className="App">
      ...
      {home ? <><div class="center">
        <AddModal handleArrayChange={handleArrayChange} />
        <button type="button" class="btn btn-success btn-margin" onClick={() => handleMinimize()}>
          MINIMIZE LATENESS
        </button>
        <button type="button" class="btn btn-info btn-margin" onClick={() => handleIntervalArray()}>
          INTERVAL SCHEDULING
        </button>
      </div>
        ...
        <> 
          <div class="center"> 
              <button type="button" class="btn btn-warning btn-margin"  onClick={() => {setHome(true); setInterval(false)}}>
          ...
          {intervalArray.map((obj)=>{
            return(
              <div class="padding"> 
                <CardActivity header= {obj.name} deadline = {obj.deadline.getDate() + '/' + padWithLeadingZeros((obj.deadline.getMonth() + 1), 2) + '/' + obj.deadline.getFullYear()} description = {obj.duration}> </CardActivity>
              </div>
            )
          })}
          </div>
        ...
        <> 
          <div class="center"> 
              <button type="button" class="btn btn-warning btn-margin"  onClick={() => {setHome(true); setMinimize(false)}}>
              Voltar para página inicial
            </button>
          </div> 
          <div class="center"> 
          ...
          {minimizeArray.map((obj)=>{
            return(
              <div class="padding"> 
                <CardActivity header= {obj.name} deadline = {obj.deadline.getDate() + '/' + padWithLeadingZeros((obj.deadline.getMonth() + 1), 2) + '/' + obj.deadline.getFullYear()} description = {obj.duration} lateness = {obj.lateness}> </CardActivity>
              </div>
            )
          })}
          </div>
